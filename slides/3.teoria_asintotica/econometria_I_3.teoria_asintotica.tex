...
\begin{document}
    ...
    \begin{frame} 
        ...
        \begin{itemize}
    		...
    		\item <2-> \term{Muestra aleatoria}: Muestra de tama\~no $N$ de la poblaci\'on obtenida en forma aleatoria y con reeemplazo $\{y_1,y_2,...,y_N\}$. \\
...
%			\uncover<5-> {Respuesta: No, en el ejemplo $\lim_{N\to\infty}\E(z_N)=1\neq 0$.}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item <4->\term{Insesgadez asint\'otica no implica consistencia, y consistencia no implica insesgadez asint\'otica.}
	...
	\begin{frame}
		\frametitle{Distribuci\'on asint\'otica normal}
		\begin{itemize}
			\item Decimos $\hat \theta_N \adistr \N(\theta, V/N)$ donde $\Avar(\hat \theta_N)=V/N$ es la \term{varianza asint\'otica de $\hat \theta_N$} y $sd(\hat \theta_N)=V^{1/2}/\sqrt{N}$ es la \term{desviaci\'on estandar asint\'otica de $\hat \theta_N$}.
		\end{itemize}
	\end{frame}
	...
	\begin{frame}
		\frametitle{Distribuci\'on asint\'otica normal}
		\begin{itemize}
			\item <1-> Un estimador $\hat \theta_N$ de un par\'ametro $\theta$ se distribuye \term{$\sqrt{N}$-asint\'oticamente normal} si,
			\begin{align*}
			\sqrt{N}(\hat \theta_N - \theta ) \dto \N(0,V),
			\end{align*}
			donde $V$ es la varianza asint\'otica de $\sqrt{N}(\hat \theta_N - \theta )$ y se denota $\Avar(\sqrt{N}(\hat \theta_N - \theta ))=V$.
	...
	\begin{frame}
		\frametitle{Teorema de Slutsky}
		\begin{itemize}
			\item \term{Ejemplos}: Suponga que $z_N \dto z \sim \N(\mu,\sigma^2)$. Encuentre la distribución de asintótica de
			\begin{enumerate}
			  \item $c_N+z_N$,
			  \item $c_N z_N$,
			  \item $\frac{z_N}{c_N}$.			  
	...
	\begin{frame}
		\frametitle{Ejemplo: demostraci\'on}
		\small
		\begin{itemize}
			\item [] Demostraci\'on: Descomponemos
			\begin{align*}
