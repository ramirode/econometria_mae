...
\begin{document}
    ...
    \begin{frame}
    	...
    	\begin{itemize}
    		\item \term{Correlaci\'on} entre $x,y$: $\Corr(x,y)=\dfrac{\Cov(x,y)}{sd(x)\,sd(y)}$
    		\begin{itemize}
    			\item Es la covarianza estandarizada, de modo que $-1 \leq \Corr(x,y) \leq 1$.
    			\item A diferencia de la covarianza, no depende de las unidades de medida:
    			\item [] $\Corr(a+bx,c+dy)=\text{signo}(bd)\,\Corr(x,y)$.
    		\end{itemize}
    		\item [] Ejemplo: si medimos $x$ en grados Fahrenheit en lugar de Celsius, $\Cov(x,y)$ cambia pero $\Corr(x,y)$ no.
    	\end{itemize}
    \end{frame}
    % --------------------------------------------------- Slide --
    \begin{frame}
